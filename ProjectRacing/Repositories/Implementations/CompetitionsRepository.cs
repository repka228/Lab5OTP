...
namespace ProjectRacing.Repositories.Implementations
{
    public class CompetitionsRepository(IConnectionString connectionString, ILogger<CompetitionsRepository> logger) : ICompetitionsRepository
    {
        ...
        private readonly ILogger<CompetitionsRepository> _logger = logger;
        public int CreateCompetitions(Competitions competitions)
        {
            ...
            try
            {
                ...
                connection.Open();
                var queryInsert = @"
            INSERT INTO Competitions (DateOfCompetitions, Adress, Name)
            VALUES (@DateOfCompetitions, @Adress, @Name)
            RETURNING Id;";
        ...
        public void UpdateCompetitions(Competitions competitions)
        {
            ...
            try
            {
                using var connection = new NpgsqlConnection(_connectionString.ConnectionString);
                connection.Open();
                var queryUpdate = @"UPDATE Competitions 
                           SET DateOfCompetitions = @DateOfCompetitions, 
                               Adress = @Adress, 
                               Name = @Name
                           WHERE Id = @Id";

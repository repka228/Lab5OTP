...
public class HorseRepository(IConnectionString connectionString, ILogger<HorseRepository> logger) : IHorseRepository
{
    ...
    public void CreateHorse(Horse horse)
    {
        ...
        try
        {
            ...
            connection.Open();
            var queryInser = @"INSERT INTO Horse (NameOfHorse, Sex, Birthday, OwnerId) VALUES (@NameOfHorse, @Sex, @Birthday, @OwnerId)";
    ...
    public void UpdateHorse(Horse horse)
    {
        ...
        try
        {
            using var connection = new NpgsqlConnection(_connectionString.ConnectionString);
            connection.Open();
            var queryUpdate = @"UPDATE Horse (NameOfHorse, Sex, Birthday, OwnerId) VALUES (@NameOfHorse, @Sex, @Birthday, @OwnerId)";
    ...
    public IEnumerable<Horse> GetHorses()
    {
        ...
        {
            using var connection = new NpgsqlConnection(_connectionString.ConnectionString);
            var querySelect = @"SELECT h.*, o.NameOfOwner AS OwnerName FROM Horse h LEFT JOIN Owner o ON h.OwnerId = o.Id";

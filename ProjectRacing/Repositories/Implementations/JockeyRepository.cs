    ...
    public class JockeyRepository(IConnectionString connectionString, ILogger<JockeyRepository> logger) : IJockeyRepository
    {
        ...
        public void CreateJockey(Jockey jockey)
        {
            ...
            try
            {
                ...
                connection.Open();
                var queryInser = @"INSERT INTO Jockey (NameOfJockey, Age, Rating, Number)
            VALUES (@NameOfJockey, @Age, @Rating, @Number)";
        ...
        public void UpdateJockey(Jockey jockey)
        {
            ...
            try
            {
                using var connection = new NpgsqlConnection(_connectionString.ConnectionString);
                connection.Open();
                var queryUpdate = @"UPDATE Jockey 
                           SET NameOfJockey = @NameOfJockey, 
                               Age = @Age, 
                               Rating = @Rating, 
                               Number = @Number
                           WHERE Id = @Id";
